(function(){
  return {
          ...
          function(result) {
            if (result) {
              ...
              hot_keys.setKeyHandler(this.hotKey,
                combats_plugins_manager.get_binded_method(this,this.doAttack),
                this.toString());
            }
          })
      );
    ...
    doAttack: function() {
      ...
      var defCount = oBattle.oAttack.rows[ 1 ].cells[ 2 ].lastChild.innerText.split(/(,| и )/).length;
      var usedBlock = {};
      ...
      switch (this.attack) {
        case 0: { // случайные атаки
          for(var i=0; i<oBattle.arrAttack.length; i++) {
            while(true) {
        ...
        case 1: { // хотя бы одна атака в зону блока
          ...
          usedBlock[attack] = true;
          oBattle.SetAttack(attack, 0);

          for(var i=1; i<oBattle.arrAttack.length; i++) {
            while(true) {
              var attack = Math.floor(Math.random()*5);
              if (this.match==0 || !usedBlock[attack])
                break;
